...
const PageQuestionnairesTwo = ({ navigation }) => {
  const [selectedOption, setSelectedOption] = useState(null);

  useEffect(() => {
    if (selectedOption) {
      const timer = setTimeout(() => {
        navigation.navigate('PageQuestionnairesThree');
      }, 1300);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.subContainer}>
        ...
        <View style={styles.radioContainer}>
          {['Homem', 'Mulher', 'Outro', 'Prefiro não dizer'].map((option, index) => (
            <TouchableOpacity
              key={index}
              style={[
                styles.radioOption,
...
export default PageQuestionnairesTwo;
